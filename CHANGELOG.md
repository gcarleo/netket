...
* Continuous-space classes ``Particle`` and ``ContinuousHilbert`` as well as the geometry utilities have moved under :mod:`netket.experimental`. Accessing them from :mod:`netket.hilbert` or :mod:`netket.geometry` emits a deprecation warning.
